...
def calculate_brightness_duration(sunrise, solar_noon, spans_count) -> int:
    spans_duration = datetime.strptime(solar_noon, TIME_FORMAT) - datetime.strptime(sunrise, TIME_FORMAT)
    spans_duration = spans_duration.total_seconds() / 60 / spans_count

    return spans_duration

def calculate_brightness(min_brightness, max_brightness, spans_duration, sunrise) -> dict:
    spans = dict()

    for i in range((max_brightness - min_brightness) * 2 + 1):
        delta = timedelta(minutes=spans_duration * i)
        brightness = int(min_brightness + (max_brightness - min_brightness) * (1 - abs(i % (100 * 2) - 100) / 100))
        time = datetime.strftime(sunrise + delta, TIME_FORMAT)
        spans[time] = brightness

    return spans

def brightness_spans_calculator(sunrise, solar_noon, min_brightness, max_brightness) -> dict:
...
if __name__ == "__main__":
    print(brightness_spans_calculator("06:45", "13:00", 0, 100))
